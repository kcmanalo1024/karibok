...
const utilities=[['Trash',Trash2],['Settings',Settings]];
...
export default function Sidebar({data,raw,page,onNavigate,collapsed,onToggle,avatar}) {
 ...
 return <aside className={`sidebar file-sidebar ${collapsed?'is-collapsed':''}`} aria-label="Workspace sidebar">
  <div className="brand"><img className="site-logo theme-logo-light" src="/Black_Logo.png" alt={collapsed?"KARIBOK":""}/><img className="site-logo theme-logo-dark" src="/White_Logo.png" alt={collapsed?"KARIBOK":""}/><div className="brand-copy"><div className="brand-wordmark" role="img" aria-label="KARIBOK"><img className="theme-logo-light" src="/karibok-wordmark-black.png" alt=""/><img className="theme-logo-dark" src="/karibok-wordmark-white.png" alt=""/></div><div className="brand-sub">study • work • hustle</div></div></div>
  ...
  <nav id="workspace-navigation" aria-label="Main navigation">{groups.map(([label,items])=><section className="nav-group" key={label} aria-label={label}><div className="nav-label">{label}</div>{items.map(item)}</section>)}<div className="mobile-nav-tools">{utilities.map(item)}</div></nav>
  <div className="sidebar-bottom"><nav aria-label="Utilities">{utilities.map(item)}</nav><button type="button" className="profile" aria-label={`Profile: ${data.name}`} title={collapsed?`Profile: ${data.name}`:undefined} onClick={()=>onNavigate('Settings')}>{avatar}<div className="profile-copy"><strong>{data.name}</strong><span>Personal workspace</span></div></button></div>
 </aside>;
}
