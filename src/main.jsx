...
import { LayoutDashboard, CheckSquare, Settings, Plus, Search, CircleCheck, Circle, Trash2, Pencil, X, Clock3, Layers, TrendingUp, Sun, Moon, Monitor, Upload, Sparkles, Folder, Star, StickyNote } from 'lucide-react';
import './styles.css';
import './depth.css';
import './v4.css';
...
import {CalendarPage,FocusPage,NotificationsPage,AccountPanel,CategoryManager,useTimer,Dialog} from './features';
...
function WorkspaceApp({workspace}) {
 ...
 const upload=e=>{const file=e.target.files?.[0];if(!file)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>2*1024*1024){setMessage('Choose a JPG, PNG, or WebP image under 2 MB.');return;}const reader=new FileReader();reader.onload=()=>update({photo:reader.result});reader.onerror=()=>setMessage('The image could not be read.');reader.readAsDataURL(file);};
 ...
 const row=t=><div className={`task-row ${t.progress===100?'done':''}`} key={t.id}><button className="check" aria-label={`${t.progress===100?'Reopen':'Complete'} ${t.title}`} onClick={()=>toggle(t.id)}>{t.progress===100?<CircleCheck size={21}/>:<Circle size={21}/>}</button><div className="task-info"><strong>{t.title}</strong><span>{data.folders.find(f=>f.id===t.folderId)?.name||t.category} · {t.progress}%</span><div className="progress-track mt-2"><div className="progress-fill" style={{width:`${t.progress}%`}}/></div></div><span className="tag">{t.category}</span><span className={`priority ${t.priority.toLowerCase()}`}>{t.priority}</span><span className={`due ${t.due&&t.due<dateKey()&&t.progress<100?'overdue':''}`}>{!t.due?'No deadline':t.due===dateKey()?'Today':new Date(t.due+'T00:00:00').toLocaleDateString(undefined,{month:'short',day:'numeric',year:new Date(t.due+'T00:00:00').getFullYear()!==new Date().getFullYear()?'numeric':undefined})}</span>{(t.subtasks||[]).length>0&&<button className="subtask-indicator" aria-label={`Open ${t.subtasks.length} subtask${t.subtasks.length===1?"":"s"} for ${t.title}`} onClick={()=>setSubtaskTask(t)}>{t.subtasks.length} subtask{t.subtasks.length===1?"":"s"}</button>}<button className="more" aria-label={`Manage subtasks ${t.title}`} onClick={()=>setSubtaskTask(t)}><Layers size={16}/></button><button className="more" aria-label={`Edit ${t.title}`} onClick={()=>edit(t)}><Pencil size={16}/></button><button className="more" aria-label={`Delete ${t.title}`} onClick={()=>remove(t.id)}><Trash2 size={16}/></button></div>;
 return <div className="app"><Sidebar data={data} raw={raw} page={page} onNavigate={setPage} collapsed={sidebarCollapsed} onToggle={toggleSidebar} avatar={avatar}/><main className={`main ${sidebarCollapsed?'sidebar-is-collapsed':''}`}><header className="topbar"><div className="search"><Search size={18}/><input aria-label="Search tasks" placeholder="Search your tasks…" value={query} onChange={e=>{setQuery(e.target.value);if(page!=='Dashboard'&&page!=='Tasks')setPage('Tasks');}}/></div><div className="header-profile"><LiveClock timeZone={data.timeZone}/><button type="button" className="icon-btn header-theme-toggle" aria-label={isDark?'Switch to light mode':'Switch to dark mode'} title={isDark?'Switch to light mode':'Switch to dark mode'} onClick={()=>update({theme:isDark?'light':'dark'})}>{isDark?<Sun size={19}/>:<Moon size={19}/>}</button><button className="icon-btn notification-bell" aria-label={`Notifications, ${notices(data).filter(n=>!data.readNotifications.includes(n.id)).length} unread`} onClick={()=>setPage('Notifications')}><Bell size={19}/>{notices(data).filter(n=>!data.readNotifications.includes(n.id)).length>0&&<span className="notification-badge">{notices(data).filter(n=>!data.readNotifications.includes(n.id)).length}</span>}</button><button className="user-pill" onClick={()=>setPage('Settings')}>{avatar}<span>{data.name}</span></button></div></header><div className="content">
 {data.timer&&page!=='Focus'&&<button className="timer-banner" onClick={()=>setPage('Focus')}><Timer size={16}/> {data.timer.startedAt?'Timer running':'Timer paused'} · {data.timer.label}<span>Open timer →</span></button>}
...
function SubtaskModal({task,onClose,onSave}){const ref=useRef(null);const [draft,setDraft]=useState(()=>normalizeSubtasks(task.subtasks));const [title,setTitle]=useState('');const [expanded,setExpanded]=useState(false);useEffect(()=>{ref.current?.showModal();return()=>{}},[]);const add=()=>{if(!title.trim())return;setDraft([...draft,{id:crypto.randomUUID(),title:title.trim(),completed:false}]);setTitle('')};const update=(i,patch)=>setDraft(draft.map((s,n)=>n===i?{...s,...patch}:s));const move=(i,dir)=>{const next=[...draft];const j=i+dir;if(j<0||j>=next.length)return;[next[i],next[j]]=[next[j],next[i]];setDraft(next)};return <dialog ref={ref} className="modal" onCancel={onClose} onClick={e=>e.target===ref.current&&onClose()}><div className="modal-head"><div><div className="eyebrow">BREAK IT DOWN</div><h2>Subtasks</h2></div><button className="icon-btn" aria-label="Close subtasks" onClick={onClose}><X size={18}/></button></div><p className="muted">{task.title} · Progress {subtasksProgress(draft)??0}%</p><div className="subtask-add"><input aria-label="New subtask" value={title} onChange={e=>setTitle(e.target.value)} onKeyDown={e=>e.key==='Enter'&&(e.preventDefault(),add())} placeholder="Add a subtask…"/><button className="secondary" onClick={add}>Add</button></div><button className="secondary subtask-toggle" onClick={()=>setExpanded(!expanded)} aria-expanded={expanded}>{expanded?"Hide":"Show"} subtasks</button>{expanded&&<div className="subtask-list">{draft.map((s,i)=><div className={`subtask-row ${s.completed?"completed":""}`} key={s.id}><input type="checkbox" aria-label={`Complete ${s.title}`} checked={s.completed} onChange={e=>update(i,{completed:e.target.checked})}/><input aria-label={`Edit ${s.title}`} value={s.title} onChange={e=>update(i,{title:e.target.value})}/><div className="subtask-actions"><button className="more" aria-label={`Move ${s.title} up`} onClick={()=>move(i,-1)}>↑</button><button className="more" aria-label={`Move ${s.title} down`} onClick={()=>move(i,1)}>↓</button><button className="more" aria-label={`Delete subtask ${s.title}`} onClick={()=>setDraft(draft.filter((_,n)=>n!==i))}><Trash2 size={15}/></button></div></div>)}</div>}{expanded&&null}<div className="modal-actions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" onClick={()=>onSave(draft)}>Save changes</button></div></dialog>}

function QuickAdd({onSelect}) {
 const [open,setOpen]=useState(false), root=useRef(null); const items=[['task','Task'],['folder','Folder'],['client','Client'],['transaction','Transaction'],['utang','Utang'],['note','Note']];
 ...
 return <div className="quick-add" ref={root} onKeyDown={keyDown}><button className="quick-add-trigger" aria-label="Quick Add" aria-expanded={open} aria-haspopup="menu" onClick={()=>setOpen(v=>!v)}><Plus size={22}/></button>{open&&<div className="quick-add-menu" role="menu" aria-label="Quick Add options">{items.map(([kind,label])=><button role="menuitem" key={kind} onClick={()=>{setOpen(false);onSelect(kind);}}>{label}</button>)}</div>}</div>;
}
