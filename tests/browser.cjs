...
(async()=>{
 ...
 await go('Clients');await go('Add client');await page.getByLabel('Client name').fill('Acme Studio');await page.getByLabel('Client email').fill('hello@example.com');await save();
 await go('Projects');await go('New project');await page.getByLabel('Project title').fill('Launch design');await page.getByLabel('Client',{exact:true}).selectOption({label:'Acme Studio'});await page.getByLabel('Description',{exact:true}).fill('Design work');await save();
 await go('Finances');await go('Add payment');await page.getByLabel('Payment description').fill('Design deposit');await page.getByLabel('Project',{exact:true}).selectOption({label:'Launch design'});await page.getByLabel('Amount (PHP)').fill('1250.50');await save();await go('Mark paid Design deposit');
 assert.match(await page.locator('.stat-grid').innerText(),/1,250.50/);
 await go('Edit Design deposit');await page.getByLabel('Amount (PHP)').fill('1500');await save();
 await go('Projects');await go('Delete Launch design');assert.match(await page.getByRole('status').innerText(),/linked tasks and payment/);await go('Dismiss notification');
 ...
 await page.waitForTimeout(1000);backend.remote={revision:backend.remote.revision+1,data:{...backend.remote.data,timer:{id:'finish-check',kind:'focus',label:'Completed focus check',duration:60,elapsed:0,startedAt:Date.now()-65000}}};await page.reload();await page.waitForTimeout(1500);await go('Notifications');assert.match(await page.locator('.content').innerText(),/Focus session complete/);await go('Dismiss Focus session complete');
 ...
 await go('Finances');await go('Delete Design deposit');await go('Tasks');await page.getByRole('button',{name:'Delete Weekly delivery',exact:true}).first().click();await page.getByRole('button',{name:'Delete Weekly delivery',exact:true}).first().click();await go('Projects');await go('Delete Launch design');await go('Clients');await go('Delete Acme Studio');
